...
def test_log_decorator_base(capsys: pytest.CaptureFixture[str]) -> None:
    assert divide_function_console(1, 2) == 0.5
    captured = capsys.readouterr()
    assert captured.out == "divide_function_console ok\n"


def test_log_decorator_exception() -> None:
    with pytest.raises(Exception):
        divide_function_console(1, 0)


def test_log_decorator_error_msg(capsys: pytest.CaptureFixture[str]) -> None:
    try:
        divide_function_console(1, 0)
...
def test_log_decorator_base_file() -> None:
