...
def get_mask_card_number(card_number: int) -> str:
    ...
    if len(str_card_number) != 16:  # нужна проверка на 16 или реализация для номеров 12 - 19 ?
        logger.error("Invalid card number: 16 digits number expected")
        raise ValueError("Invalid card number")
    result = str_card_number[0:4] + " " + str_card_number[4:6] + "** **** " + str_card_number[-4:]
...
def get_mask_account(account_number: int) -> str:
    ...
    if len(str(account_number)) != 20:  # или номер может быть другой длинны?
        logger.error("Invalid account number: 20 digits number expected")
        raise ValueError("Invalid card number")
